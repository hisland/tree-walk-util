...
import { treeDeepFind } from './treeWalkDeep'
...
describe('treeWalkDeep', () => {
  ...
  for (const [name, val2] of arr) {
    describe(name, () => {
      it('lv && index', () => {
        treeWalkDeep(val2, function (val, index, list, parent, lv) {
  ...
  it('raw value loop', () => {
    ...
    for (const val of list) {
      let times = 0
      let that
      treeWalkDeep((val as unknown) as object, function (
        val,
        index,
        list,
        parent,
...
describe('treeDeepFind', () => {
  ...
  for (const [name, val2] of arr) {
    describe(name, () => {
      it('run times 1', () => {
        let times = 0
        treeDeepFind(val2, function (val, index, list, parent, lv) {
          times++
          if (val.foo === 'bar') {
            return true
          }
